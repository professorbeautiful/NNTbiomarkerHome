...
NNTconfint = function(
  ...
  binom.confint(tp, Npositives)  #CI for SN
  (nntPosPI = 1/ppvPI[2:1])
  
  (npvPI = qbeta(c(alpha, 1-alpha), 
                 tn+prior[1]-1, fp+prior[2]-1))
  binom.confint(tn, Nnegatives)  #CI for SP
  (nntNegPI = 1/(1-npvPI))
...
(sesp = sesp.from.pv(ppv = ppv, npv = npv, prev=prev))
  ...
  (sesp.from.NNT(NNTpos = nntlower, NNTneg=nntupper, prev=prev))
  # same
  snHat = sesp["se"]
  tp = round(22*snHat)
  fn = round(22*(1-snHat))
  ...
  abline(v=NNTlower, col="blue")
  plot(density(nntNeg), xlab=NA,main=NA, xlim=c(1,70))
  abline(v=NNTupper, col="blue")
