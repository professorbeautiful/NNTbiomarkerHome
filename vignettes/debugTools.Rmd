...
On the right, you can evaluate an R expression. The use of capture.output allows more complex objects unfriendly to cat(), in particular lists, to be viewed. You can for example debug functions with debug(), and cancel with undebug(). You can change values at the GlobalEnv level. Note that this is not the same GlobalEnv as the one in your interactive RStudio session; changes do not propagate back.
